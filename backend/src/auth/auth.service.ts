...
import { AuthStateDto, AuthTypeEnum } from './dto/AuthStateDto';
...
export class AuthService {
  ...
  async signIn(email: string, pass: string,response: Response) {
    ...
    const authState = new AuthStateDto(user,access_token,AuthTypeEnum.AUTHENTICATED)
  ...
  async getAuthState(request: Request): Promise<AuthStateDto>{
    ...
    if(!jwtToken) return new AuthStateDto(null,null,AuthTypeEnum.ANONYMOUS)
    try{
      const payload = await this.jwtService.verifyAsync(jwtToken,{secret: process.env.JWT_SECRET})
      const user = await this.userService.getById(payload.sub);
      return new AuthStateDto(user,jwtToken,AuthTypeEnum.AUTHENTICATED)
    }catch(e) {
